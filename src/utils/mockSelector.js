...
/**
 * MOCK TEST SELECTOR (200 Questions)
 */
export async function getMockQuestions(subject, count, seedStr) {
  try {
    ...
    console.error('Mock Selector Error:', error);
    return [];
  }
}

export async function loadMockTest(config, date) {
  const seed = `${date}_A`;
  const subjects = ['english', 'reasoning', 'gk', 'computer'];
  const fetchPromises = subjects.map(sub =>
    getMockQuestions(sub, config.distribution[sub], seed)
  );
  const results = await Promise.all(fetchPromises);
  return results.flat();
}
